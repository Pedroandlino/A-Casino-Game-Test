...
bool WindowManager::createWindow(const std::string& title, const sf::Vector2u& dims, int fps, const std::string& iconPath)
{
	if (getInstance().m_windowModelMap.count(title) == 0) {
		getInstance().m_windowModelMap[title] =
			boost::shared_ptr<WindowModel>(new WindowModel(title, dims));
		...
		if (!iconPath.empty()) {
			...
			if (icon.loadFromFile(iconPath)) {
				getInstance().m_windowModelMap[title]->setIcon(icon.getSize().x,icon.getSize().y, icon.getPixelsPtr());
			}
			else {
...
boost::shared_ptr<WindowModel> WindowManager::getWindowModel(const std::string& title)
{
	if (getInstance().m_windowModelMap.count(title) == 0)
	{
		return nullptr;
