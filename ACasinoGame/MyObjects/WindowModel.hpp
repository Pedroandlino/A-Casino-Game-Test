...
public:
	...
	*/
	 ...
	 */
	void addChild(std::shared_ptr<WindowInterface> child);

	/**
	 * @brief Method which adds a child button to be rendered on window.
	 * @param buttons The child shape of ButtonInterface type.
	 */
	void removeButtons(std::vector<std::shared_ptr<ButtonInterface>> buttons);
...
private:
	...
	std::vector<std::shared_ptr<WindowInterface>> m_children;

	/** @brief Holds the vector of ButtonInterface references (not the owner). */
	std::vector<std::shared_ptr<ButtonInterface>> m_buttons;
};